...
$(document).ready(function () {
    ...
    /*****************************Nemeric Only**********************************************************/
    $(".integerOnly").live('keydown', function (event) {
    ...
    $(".NumericOnly").live("keypress", function (e) {
        var key;
        key = e.which ? e.which : e.keyCode;
    ...
    $("input[type='text']:not(.NoValidation), input[type='password']:not(.NoValidation)").live("keyup", function () {
        ...
        if ($(this).hasClass('AlphaNumericOnly') && ReturnMessage == '') {
            ...
            ReturnMessage = !isAlphaNumeric($(this).val()) ? 'numbers and/or characters, like 123abc' : '';
        }
        ...
        if ($(this).hasClass('ValidPhone') && ReturnMessage == '') {
            cls += "#ValidPhone";
            ReturnMessage = !isValidPhone($(this).val()) ? 'Please enter phone number in ###-###-#### format.' : '';
        }
...
function TestValidation(obj, showMessage) {
    ...
    $(".AlphaNumericOnly", $("#" + obj.toString()).parents(".form:eq(0)")).each(function () {
        if (!isAlphaNumeric($(this).val()) && IsValid) {
            AddErrorTag($(this), "");
            ReturnMessage = 'only numbers and/or characters, like 123abc are allowed';
            controlWithError = $(this);
            IsValid = false;
        }
    });
    ...
    $(".ValidPhone", $("#" + obj.toString()).parents(".form:eq(0)")).each(function () {
        if (!isValidPhone($(this).val()) && IsValid) {
            AddErrorTag($(this), "");
            ReturnMessage = 'only phone number in ###-###-#### format.';
            controlWithError = $(this);
            IsValid = false;
        }
    });
...
function isDecimal(strVal) {
    return (checkValidation(strVal, "^[-+]?\d*\.?\d*$"));
}
...
function isValidUrl(strVal) {
    ...
    return checkValidation(strVal, "^(www\.|http:\/\/|https:\/\/|http:\/\/www\.|https:\/\/www\.)[a-z0-9]+\.[a-z]{2,4}$");
...
function ValidateValue(control) {
    ...
    if (control.hasClass('AlphaNumericOnly') && ReturnMessage == '') {
        cls += "#AlphaNumericOnly";
        ReturnMessage = !isAlphaNumeric(control.val()) ? 'numbers and/or characters, like 123abc' : '';
    }
    ...
    if (control.hasClass('ValidPhone') && ReturnMessage == '') {
        cls += "#ValidPhone";
        ReturnMessage = !isValidPhone(control.val()) ? 'Please enter phone number in ###-###-#### format.' : '';
    }
